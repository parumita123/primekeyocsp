...
public class OcspCheckerImplTest {
    private final OcspCheckerImpl ocspChecker = new OcspCheckerImpl();
    private final byte[] ocspValidByteInput = {48, -126, 3, -71, 10, 1, 0, -96, -126, 3, -78,
    ...
    @Test
    void ocspConverter_validResponse() {
        BasicOCSPResp basicOCSPResp = ocspChecker.ocspConverter(ocspValidByteInput);
        assertNotNull(basicOCSPResp);
    }

    @Test
    void ocspConverter_invalidResponse() {
        BasicOCSPResp basicOCSPResp = ocspChecker.ocspConverter(null);
        assertNull(basicOCSPResp);
    }

    @Test
    void verifyOcspSignature_validSignature() {
        boolean status = ocspChecker.verifyOcspSignature(ocspChecker.ocspConverter(ocspValidByteInput));
        assertEquals(true, status);
    }

    @Test
    void verifyOcspSignature_withNull_invalidSignature() {
        BasicOCSPResp basicOCSPResp = mock(BasicOCSPResp.class);
        when(basicOCSPResp.getCerts()).thenReturn(null);

        boolean status = ocspChecker.verifyOcspSignature(basicOCSPResp);
        assertEquals(false, status);
    }

    @Test
    void verifyOcspNonce_validNonce() {
        boolean status = ocspChecker.verifyOcspNonce(ocspChecker.ocspConverter(ocspValidByteInput));
        assertEquals(true, status);
    }
}